...
{
	TEST_CLASS(SinglyLinkedListWithHeadTest)
	{
	private:
		const int n = 10;
		const unsigned seed = clock();
		SinglyLinkedListWithHead<int> *L;

	public:
		TEST_METHOD_INITIALIZE(SetUp)
		{
			L = new SinglyLinkedListWithHead<int>();
			srand(seed);
			for (int i = 0; i < n; i++)
			{
				L->Add(rand());
		...
		TEST_METHOD(TestAdd)
		{
			SinglyLinkedListWithHead<int> testL;
			srand(seed);
			for (int i = 0; i < n; i++)
			{
				testL.Add(rand());
		...
		{
			SinglyLinkedListWithHead<int> testL(*L);
			Assert::IsTrue(testL == *L);
			testL[0]++;
			Assert::IsFalse(testL == *L);
			testL[0]--;
