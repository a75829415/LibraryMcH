...
{
	TEST_CLASS(CircularSinglyLinkedListWithHeadTest)
	{
	private:
		const int n = 10;
		const unsigned seed = clock();
		CircularSinglyLinkedListWithHead<int> *L;

	public:
		TEST_METHOD_INITIALIZE(SetUp)
		{
			L = new CircularSinglyLinkedListWithHead<int>();
			srand(seed);
			for (int i = 0; i < n; i++)
			{
				L->Add(rand());
		...
		TEST_METHOD(TestAdd)
		{
			CircularSinglyLinkedListWithHead<int> testL;
			srand(seed);
			for (int i = 0; i < n; i++)
			{
				testL.Add(rand());
		...
		{
			CircularSinglyLinkedListWithHead<int> testL(*L);
			Assert::IsTrue(testL == *L);
			testL[0]++;
			Assert::IsFalse(testL == *L);
			testL[0]--;
