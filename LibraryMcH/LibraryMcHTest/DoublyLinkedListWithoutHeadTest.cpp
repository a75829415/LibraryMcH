...
{
	TEST_CLASS(DoublyLinkedListWithoutHeadTest)
	{
	private:
		const int n = 10;
		const unsigned seed = clock();
		DoublyLinkedListWithoutHead<int> *L;

	public:
		TEST_METHOD_INITIALIZE(SetUp)
		{
			L = new DoublyLinkedListWithoutHead<int>();
			srand(seed);
			for (int i = 0; i < n; i++)
			{
				L->Add(rand());
		...
		TEST_METHOD(TestAdd)
		{
			DoublyLinkedListWithoutHead<int> testL;
			srand(seed);
			for (int i = 0; i < n; i++)
			{
				testL.Add(rand());
		...
		{
			DoublyLinkedListWithoutHead<int> testL(*L);
			Assert::IsTrue(testL == *L);
			testL[0]++;
			Assert::IsFalse(testL == *L);
			testL[0]--;
