...
{
	TEST_CLASS(SinglyLinkedListWithoutHeadTest)
	{
	private:
		const int n = 10;
		const unsigned seed = clock();
		SinglyLinkedListWithoutHead<int> *L;

	public:
		TEST_METHOD_INITIALIZE(SetUp)
		{
			L = new SinglyLinkedListWithoutHead<int>();
			srand(seed);
			for (int i = 0; i < n; i++)
			{
				L->Add(rand());
		...
		TEST_METHOD(TestAdd)
		{
			SinglyLinkedListWithoutHead<int> testL;
			srand(seed);
			for (int i = 0; i < n; i++)
			{
				testL.Add(rand());
		...
		{
			SinglyLinkedListWithoutHead<int> testL(*L);
			Assert::IsTrue(testL == *L);
			testL[0]++;
			Assert::IsFalse(testL == *L);
			testL[0]--;
