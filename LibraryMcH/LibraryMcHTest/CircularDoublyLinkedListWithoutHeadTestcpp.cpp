...
{
	TEST_CLASS(CircularDoublyLinkedListWithoutHeadTest)
	{
	private:
		const int n = 10;
		const unsigned seed = clock();
		CircularDoublyLinkedListWithoutHead<int> *L;

	public:
		TEST_METHOD_INITIALIZE(SetUp)
		{
			L = new CircularDoublyLinkedListWithoutHead<int>();
			srand(seed);
			for (int i = 0; i < n; i++)
			{
				L->Add(rand());
		...
		TEST_METHOD(TestAdd)
		{
			CircularDoublyLinkedListWithoutHead<int> testL;
			srand(seed);
			for (int i = 0; i < n; i++)
			{
				testL.Add(rand());
		...
		{
			CircularDoublyLinkedListWithoutHead<int> testL(*L);
			Assert::IsTrue(testL == *L);
			testL[0]++;
			Assert::IsFalse(testL == *L);
			testL[0]--;
